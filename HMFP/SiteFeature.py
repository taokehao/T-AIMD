...
def average_bond_length(structure, icon):
    """
    ...
    index = structure.indices_from_symbol(icon)
...
def average_bond_angle(structure, icon):
    """
    :param structure: a Pymatgen Structure
...
def average_neighbor_num(structure, icon):
    """
    :param structure: a Pymatgen Structure
    :return: average number of neighbors for all Li sites
    """
    sites = [site for site in structure.sites if site.specie.symbol == icon]
...
def angular_fourier_series(structure, icon, show=0):
    """
    :param structure: a Pymatgen Structure
...
def effective_location(structure, icon):
    """
    :param structure: a Pymatgen Structure
    ...
    for i in range(0, 1000): # 1000 random points
        ...
        for j in range(0, len(all_sites)):
            ...
            if distance < specie.average_ionic_radius + Element(icon).average_ionic_radius + 0.25:  # Li: 0.9
                count -= 1
                break
