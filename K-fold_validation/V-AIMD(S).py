...
# 导入训练数据
data = np.loadtxt("../data/feature_encoder_vae.csv", delimiter=',')
...
for train, test in kf.split(data):
    ...
    model = cat.CatBoostRegressor(iterations=1500, learning_rate=0.01, max_depth=10, subsample=0.7)
    ...
    csvFile = open("./cal-pre-data/V-AIMD(S)"+str(index)+".csv", 'a', newline='', encoding='utf-8')
